...
async fn status_transitions() {
    ...
    let task = hub
        .tasks()
        .set_status("TASK-001", TaskStatus::Ready)
        .await
        .unwrap();
    ...
    let task = hub
        .tasks()
        .set_status("TASK-001", TaskStatus::InProgress)
        .await
        .unwrap();
    ...
    let task = hub
        .tasks()
        .set_status("TASK-001", TaskStatus::Done)
        .await
        .unwrap();
...
async fn blocked_sets_paused() {
    ...
    let task = hub
        .tasks()
        .set_status("TASK-001", TaskStatus::Blocked)
        .await
        .unwrap();
    assert!(task.paused);
...
async fn checklist_operations() {
    ...
    let task = hub
        .tasks()
        .add_checklist_item("TASK-001", "Step 1".into())
        .await
        .unwrap();
    ...
    let task = hub
        .tasks()
        .update_checklist_item("TASK-001", &item_id, true)
        .await
        .unwrap();
    assert!(task.checklist[0].completed);
...
async fn dependency_management() {
    ...
    let task = hub
        .tasks()
        .add_dependency("TASK-002", "TASK-001", DependencyType::BlockedBy)
        .await
        .unwrap();
    ...
    let task = hub
        .tasks()
        .remove_dependency("TASK-002", "TASK-001")
        .await
        .unwrap();
