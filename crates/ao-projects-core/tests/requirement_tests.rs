...
async fn create_and_get_requirement() {
    ...
    let req = hub.requirements().create(RequirementCreateInput {
...
async fn refine_requirement() {
    let hub = ProjectHub::in_memory();
    hub.requirements().create(RequirementCreateInput {
...
async fn filter_by_status() {
    let hub = ProjectHub::in_memory();
    hub.requirements().create(RequirementCreateInput { title: "A".into(), ..Default::default() }).await.unwrap();
    hub.requirements().create(RequirementCreateInput { title: "B".into(), ..Default::default() }).await.unwrap();
    hub.requirements().refine("REQ-001").await.unwrap();

    let refined = hub.requirements().list(Some(RequirementFilter {
...
async fn filter_by_category() {
    ...
    let security = hub.requirements().list(Some(RequirementFilter {
        category: Some("security".into()),
        ..Default::default()
    })).await.unwrap();
...
async fn update_requirement() {
    let hub = ProjectHub::in_memory();
    hub.requirements().create(RequirementCreateInput { title: "Original".into(), ..Default::default() }).await.unwrap();

    let req = hub.requirements().update("REQ-001", RequirementUpdateInput {
...
async fn bidirectional_linking() {
    ...
    let task = hub.create_task_linked(TaskCreateInput {
...
async fn delete_requirement() {
    let hub = ProjectHub::in_memory();
    hub.requirements().create(RequirementCreateInput { title: "Doomed".into(), ..Default::default() }).await.unwrap();
